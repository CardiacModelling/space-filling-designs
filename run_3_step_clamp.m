...
start_time = 0;
end_time = duration1;
[t1,states1] = ode15s(@model,[start_time:1:end_time],y0,options,[0 V_step1; duration1 V_step1], Model_Params, Protocol_Params);
V1 = ones(length(t1),1).*V_step1;

start_time = duration1;
end_time = duration1+duration2;

[t2,states2] = ode15s(@model,[start_time:1:end_time],states1(end,:),options,[start_time V_step2; end_time V_step2], Model_Params, Protocol_Params);
V2 = ones(length(t2),1).*V_step2;

start_time = duration1+duration2;
end_time = duration1+duration2+duration3;
[t3,states3] = ode15s(@model,[start_time:1:end_time],states2(end,:),options,[start_time V_step3; end_time V_step3], Model_Params, Protocol_Params);
